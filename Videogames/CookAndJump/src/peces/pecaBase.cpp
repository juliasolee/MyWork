...
void pecaBase::update(int total, ofVec2f pos[MAX_NUM_BLOBS], JointType type[MAX_NUM_BLOBS]){
    if(bpecaActiva){
        // DETECCIO
        for(int i=0; i<total; i++){
			if (estatPeca == IDLE) {
				if (type[i] == JOINT_LEFT_HAND || type[i] == JOINT_RIGHT_HAND) {
...
void pecaBase::updatem(ofVec2f pos, JointType type){
    // MOUSE
    if(bpecaActiva){
        if(estatPeca == IDLE){
			if (type == JOINT_LEFT_HAND || type == JOINT_RIGHT_HAND) {
